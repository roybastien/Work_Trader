...
<div class="forumbg forumbg-table">
	...
	<thead>
	<tr>
		<th class="name"><?php echo ((isset($this->_rootref['L_ADMINISTRATORS'])) ? $this->_rootref['L_ADMINISTRATORS'] : ((isset($user->lang['ADMINISTRATORS'])) ? $user->lang['ADMINISTRATORS'] : '{ ADMINISTRATORS }')); ?></th>
		<th class="rank"><span class="rank-img"><?php echo ((isset($this->_rootref['L_RANK'])) ? $this->_rootref['L_RANK'] : ((isset($user->lang['RANK'])) ? $user->lang['RANK'] : '{ RANK }')); ?>&nbsp;</span></th>
...
<?php $_admin_count = (isset($this->_tpldata['admin'])) ? sizeof($this->_tpldata['admin']) : 0;if ($_admin_count) {for ($_admin_i = 0; $_admin_i < $_admin_count; ++$_admin_i){$_admin_val = &$this->_tpldata['admin'][$_admin_i]; ?>
	...
	<tr class="<?php if (!($_admin_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } ?>">
		<td><?php echo $_admin_val['USERNAME_FULL']; ?></td>
		<td><?php if ($_admin_val['RANK_IMG']) {  ?><span class="rank-img"><?php echo $_admin_val['RANK_IMG']; ?></span><?php } else { ?><span class="rank-img"><?php echo $_admin_val['RANK_TITLE']; ?></span><?php } ?></td>
...
<?php }} else { ?>
	...
	<tr class="bg1">
		<td colspan="4"><strong><?php echo ((isset($this->_rootref['L_NO_ADMINISTRATORS'])) ? $this->_rootref['L_NO_ADMINISTRATORS'] : ((isset($user->lang['NO_ADMINISTRATORS'])) ? $user->lang['NO_ADMINISTRATORS'] : '{ NO_ADMINISTRATORS }')); ?></strong></td>
	</tr>
<?php } ?>
...
<div class="forumbg forumbg-table">
	...
	<thead>
	<tr>
		<th class="name" colspan="4"><?php echo ((isset($this->_rootref['L_MODERATORS'])) ? $this->_rootref['L_MODERATORS'] : ((isset($user->lang['MODERATORS'])) ? $user->lang['MODERATORS'] : '{ MODERATORS }')); ?></th>
...
<?php $_mod_count = (isset($this->_tpldata['mod'])) ? sizeof($this->_tpldata['mod']) : 0;if ($_mod_count) {for ($_mod_i = 0; $_mod_i < $_mod_count; ++$_mod_i){$_mod_val = &$this->_tpldata['mod'][$_mod_i]; ?>
	...
	<tr>
		<td><?php echo $_mod_val['USERNAME_FULL']; ?></td>
		<td><?php if ($_mod_val['RANK_IMG']) {  ?><span class="rank-img"><?php echo $_mod_val['RANK_IMG']; ?></span><?php } else { ?><span class="rank-img"><?php echo $_mod_val['RANK_TITLE']; ?></span><?php } ?></td>
...
<?php }} else { ?>
	...
	<tr class="bg1">
		<td colspan="4"><strong><?php echo ((isset($this->_rootref['L_NO_MODERATORS'])) ? $this->_rootref['L_NO_MODERATORS'] : ((isset($user->lang['NO_MODERATORS'])) ? $user->lang['NO_MODERATORS'] : '{ NO_MODERATORS }')); ?></strong></td>
	</tr>
<?php } ?>
