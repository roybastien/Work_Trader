<?php if (!defined('IN_PHPBB')) exit; ?><div class="panel panel-default">
	...
	<div id="topicreview" class="panel-body no-margin-bottom">
		...
		<?php $_topic_review_row_count = (isset($this->_tpldata['topic_review_row'])) ? sizeof($this->_tpldata['topic_review_row']) : 0;if ($_topic_review_row_count) {for ($_topic_review_row_i = 0; $_topic_review_row_i < $_topic_review_row_count; ++$_topic_review_row_i){$_topic_review_row_val = &$this->_tpldata['topic_review_row'][$_topic_review_row_i]; if ($_topic_review_row_val['S_IGNORE_POST']) {  ?>
			...
			<?php } ?>
				...
				<div id="pr<?php echo $_topic_review_row_val['POST_ID']; ?>" class="preview-item">
						...
						<div class="panel-heading btn-panel">
							...
							<span class="pull-right btn-group panel-right">
								<?php if ($_topic_review_row_val['U_MCP_DETAILS']) {  ?>
									...
									<a href="<?php echo $_topic_review_row_val['U_MCP_DETAILS']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-info"></i></a>
								<?php } if ($_topic_review_row_val['POSTER_QUOTE'] && $_topic_review_row_val['DECODED_MESSAGE']) {  ?>

									<a href="#postingbox" class="btn btn-success btn-sm" onclick="addquote(<?php echo $_topic_review_row_val['POST_ID']; ?>, '<?php echo $_topic_review_row_val['POSTER_QUOTE']; ?>', '<?php echo ((isset($this->_rootref['LA_WROTE'])) ? $this->_rootref['LA_WROTE'] : ((isset($this->_rootref['L_WROTE'])) ? addslashes($this->_rootref['L_WROTE']) : ((isset($user->lang['WROTE'])) ? addslashes($user->lang['WROTE']) : '{ WROTE }'))); ?>');" title="<?php echo ((isset($this->_rootref['L_QUOTE'])) ? $this->_rootref['L_QUOTE'] : ((isset($user->lang['QUOTE'])) ? $user->lang['QUOTE'] : '{ QUOTE }')); ?> <?php echo $_topic_review_row_val['POST_AUTHOR']; ?>"><span><i class="fa fa-quote-left"></i></span></a>
								<?php } ?>
	...
	<div class="panel-footer review-footer">
		<?php if ($this->_rootref['S_MCP_REPORT']) {  ?>
			...
			<div class="to-top pull-right"><a href="#report" class="top"><i class="fa fa-chevron-up"></i></a></div>
		<?php } else { ?>

			<div class="to-top pull-right"><a href="#postingbox" class="top"><i class="fa fa-chevron-up"></i></a></div>
		<?php } ?>
