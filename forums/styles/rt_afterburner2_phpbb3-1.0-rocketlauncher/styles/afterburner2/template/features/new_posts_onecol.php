...
*
* @package module
* @version $Id: new_posts_onecol.php 8111 2013-03-07 16:54:05Z karol $
...
{ 
    $counter++; 
    if(($counter % $cells_per_row) == 1)  { echo '<ul class="list">'; }
    $url = append_sid(generate_board_url() . "/viewtopic.{$phpEx}?f={$row['forum_id']}&amp;t={$row['topic_id']}&amp;p={$row['post_id']}#p{$row['post_id']}");
    echo '<li class="icon-plus"><a href="' . $url . '">' . $row['post_subject'] . '</a>&nbsp;'. $user->lang['POST_BY_AUTHOR'] .'&nbsp;'. $row['username'] . '</li>'; 
    if(($counter %  $cells_per_row) == 0) { echo '</ul>'; } 
} 
