<?php
/**
 * @version   $Id: LessCompiler.php 9497 2013-04-19 16:25:44Z btowles $
 ...
 * by `RokBB_LessCompiler_Parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `RokBB_LessCompiler`. The CSS tree is fed into a
 ...
 *
 * The `parse` function of `RokBB_LessCompiler` is the entry point.
 *
 * In summary:
 *
 * The `RokBB_LessCompiler` class creates an intstance of the parser, feeds it LESS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `RokBB_LessCompiler_Parser` class is only concerned with parsing its input.
 *
 * The `lessc_formatter` takes a CSS tree, and dumps it to a formatted string,
 * handling things like indentation.
 */
class RokBB_LessCompiler {
	 ...
	 *
	 * See RokBB_LessCompiler::compileProp()
	 *
	 */
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new RokBB_LessCompiler_Parser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new RokBB_LessCompiler_Parser($this, $name);
	...
	protected function newFormatter() {
		$className = "RokBB_LessCompiler_Formatter_Lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "RokBB_LessCompiler_Formatter_$this->formatterName";
		}
...
// syntax tree
class RokBB_LessCompiler_Parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('RokBB_LessCompiler', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = RokBB_LessCompiler::preg_quote(self::$commentSingle);
			$commentMultiLeft = RokBB_LessCompiler::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = RokBB_LessCompiler::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function RokBB_LessCompiler::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by RokBB_LessCompiler::match(), which behaves
	 ...
	 * avoid this, RokBB_LessCompiler::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("RokBB_LessCompiler", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = RokBB_LessCompiler::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = RokBB_LessCompiler::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("RokBB_LessCompiler", "preg_quote"), $stop);
	...
	protected function string(&$out) {
		...
		$patt = '([^\n]*?)(@\{|\\\\|' .
			RokBB_LessCompiler::preg_quote($delim).')';
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = RokBB_LessCompiler::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.RokBB_LessCompiler::preg_quote($what), $m, !$until)) return false;
...
class RokBB_LessCompiler_Formatter_Classic {
	public $indentChar = "  ";
...
class RokBB_LessCompiler_Formatter_Compressed extends RokBB_LessCompiler_Formatter_Classic {
...
class RokBB_LessCompiler_Formatter_Lessjs extends RokBB_LessCompiler_Formatter_Classic {
	public $disableSingle = true;
